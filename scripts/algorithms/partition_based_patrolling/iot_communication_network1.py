...
class MRPP_IOT:
    def __init__(self, graph):
        ...
        self.algo_name = 'iot_communication_network1'
        

        self.total_base_stations = self.base_stations_df.shape[0]
        self.total_nodes = len(self.nodes)
        self.base_stations_arr = np.zeros([self.total_base_stations,self.total_nodes]) # A virtual array for storing data of all IoT devices (Base stations)
        self.agents_arr = np.zeros([self.num_bots,self.total_nodes])  # A virtual array of agent for storing data of all IoT devices (Base stations)
    ...
    def callback_next_task(self, req):
        ...
        temp = np.ones(self.total_nodes) * np.inf
        ...
        for base_station_id in which_base_stations:
            ...
            temp = np.minimum(self.base_stations_arr[base_station_id, :], temp)
    ...
    def callback_ready(self, req):
        algo_name = req.algo
        if algo_name == 'iot_communication_network1' and self.ready:
